...
/// <summary>
/// Covers SeedDirectoryTaskRunner reading source_resource rows for the task's target CorrelationId (carried as the
/// raw Guid in SeedDirectoryTask.MetaData, distinct from the run's own correlationId) and deserialising each into a
/// Firely POCO.
/// </summary>
public class SeedDirectoryTaskRunnerTests
{
    ...
    private static SeedDirectoryTaskRunner.SeedDirectoryTaskRunner NewRunner(
        FakeSourceResourceRepository repository)
    {
        return new SeedDirectoryTaskRunner.SeedDirectoryTaskRunner(
            NullLogger<SeedDirectoryTaskRunner.SeedDirectoryTaskRunner>.Instance,
            repository);
    }
    ...
    public async Task Run_QueriesAllSixResourceTypes_UsingTheCorrelationIdFromMetaData_NotTheRunsOwnCorrelationId()
    {
        ...
        SeedDirectoryTaskRunner.SeedDirectoryTaskRunner runner = NewRunner(repository);
        SeedDirectoryTask seedDirectoryTask = NewSeedDirectoryTask();

        await runner.Run(seedDirectoryTask, RunCorrelationId, CancellationToken.None);
    ...
    public async Task Run_DeserialisesEachSourceResourceJsonIntoAFirelyPoco_AndCountsIt()
    {
        ...
        SeedDirectoryTaskResult result = await runner.Run(NewSeedDirectoryTask(), RunCorrelationId, CancellationToken.None);
